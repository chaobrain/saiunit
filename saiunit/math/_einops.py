...
def einsum(
    ...
    **Vector product**
    ...
    >>> u.math.einsum('i,i->', x, y)  # explicit form
    16.0 uW
    >>> u.math.einsum(x, (0,), y, (0,))  # implicit form via indices
    16.0 uW
    >>> u.math.einsum(x, (0,), y, (0,), ())  # explicit form via indices
    16.0 uW

    **Matrix product**

    >>> u.math.einsum('ij,j->i', M, x)  # explicit form
    ...
    **Outer product**
    ...
    >>> u.math.einsum("i,j", x, y)  # implicit form
    Array([[ 0,  0,  0,  0],
             [ 5,  4,  3,  2],
             [10,  8,  6,  4],
             [15, 12,  9,  6]], dtype=float32) * uwatt
    >>> u.math.einsum(x, (0,), y, (1,), (0, 1))  # explicit form via indices
    Array([[ 0,  0,  0,  0],
             [ 5,  4,  3,  2],
             [10,  8,  6,  4],
             [15, 12,  9,  6]], dtype=float32) * uwatt
    >>> u.math.einsum(x, (0,), y, (1,))  # implicit form via indices
    Array([[ 0,  0,  0,  0],
             [ 5,  4,  3,  2],
             [10,  8,  6,  4],
             [15, 12,  9,  6]], dtype=float32) * uwatt
    ...
    **1D array sum**
    ...
    >>> u.math.einsum("i->", x)  # requires explicit form
    Array(6, dtype=float32) * mA
    >>> u.math.einsum(x, (0,), ())  # explicit form via indices
    Array(6, dtype=float32) * mA
    >>> u.math.sum(x)
    Array(6, dtype=float32) * mA
    ...
    **Sum along an axis**
    ...
    >>> u.math.einsum("...j->...", M)  # requires explicit form
    Array([ 6, 22, 38, 54], dtype=float32) * ohm
    >>> u.math.einsum(M, (..., 0), (...,))  # explicit form via indices
    Array([ 6, 22, 38, 54], dtype=float32) * ohm
    >>> M.sum(-1)
    Array([ 6, 22, 38, 54], dtype=float32) * ohm
    ...
    **Matrix transpose**
    ...
    >>> y = u.math.array([[1, 2, 3],
    ...                    [4, 5, 6]]) * u.mV
    >>> u.math.einsum("ij->ji", y)  # explicit form
    Array([[1, 4],
             [2, 5],
             [3, 6]], dtype=float32) * mV
    >>> u.math.einsum("ji", y)  # implicit form
    Array([[1, 4],
             [2, 5],
             [3, 6]], dtype=float32) * mV
    >>> u.math.einsum(y, (1, 0))  # implicit form via indices
    Array([[1, 4],
             [2, 5],
             [3, 6]], dtype=float32) * mV
    >>> u.math.einsum(y, (0, 1), (1, 0))  # explicit form via indices
    Array([[1, 4],
             [2, 5],
             [3, 6]], dtype=float32) * mV
    >>> u.math.transpose(y)
    ...
    **Tensor products**
    ...
    >>> u.math.einsum('ijk,jlk->il', x, y)  # explicit form
    Array([[ 3340,  3865,  4390,  4915],
             [ 8290,  9940, 11590, 13240]], dtype=float32) * volt
    >>> u.math.tensordot(x, y, axes=((1, 2), (0, 2)))
    Array([[ 3340,  3865,  4390,  4915],
             [ 8290,  9940, 11590, 13240]], dtype=float32) * volt
    >>> u.math.einsum('ijk,jlk', x, y)  # implicit form
    Array([[ 3340,  3865,  4390,  4915],
             [ 8290,  9940, 11590, 13240]], dtype=float32) * volt
    >>> u.math.einsum(x, (0, 1, 2), y, (1, 3, 2), (0, 3))  # explicit form via indices
    Array([[ 3340,  3865,  4390,  4915],
             [ 8290,  9940, 11590, 13240]], dtype=float32) * volt
    >>> u.math.einsum(x, (0, 1, 2), y, (1, 3, 2))  # implicit form via indices
    Array([[ 3340,  3865,  4390,  4915],
             [ 8290,  9940, 11590, 13240]], dtype=float32) * volt
    ...
    **Chained dot products**
    ...
    >>> u.math.einsum('ij,jk,kl,lm->im', w, x, y, z)
    Array([[ 481,  831, 1181],
             [ 651, 1125, 1599]], dtype=float32) * metre ** 4 * kilogram ** 2 * second ** -6 * amp ** -1
    >>> u.math.einsum(w, (0, 1), x, (1, 2), y, (2, 3), z, (3, 4))  # implicit, via indices
    Array([[ 481,  831, 1181],
             [ 651, 1125, 1599]], dtype=float32) * metre ** 4 * kilogram ** 2 * second ** -6 * amp ** -1
    >>> w @ x @ y @ z  # direct chain of matmuls
    Array([[ 481,  831, 1181],
             [ 651, 1125, 1599]], dtype=float32) * metre ** 4 * kilogram ** 2 * second ** -6 * amp ** -1
    >>> u.math.multi_dot([w, x, y, z])
    Array([[ 481,  831, 1181],
             [ 651, 1125, 1599]], dtype=float32) * metre ** 4 * kilogram ** 2 * second ** -6 * amp ** -1
